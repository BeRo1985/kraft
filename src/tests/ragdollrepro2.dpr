...
var UseBoxArms:boolean=false;
    ElbowKneeAsBallSocket:boolean=false;
...
begin
 ...
 if UseBoxArms then begin
  RagdollBodies[3]:=CreateBoxBody(Physics,Vector3(0.20,0.05,0.05),Matrix4x4Translate(0.36,1.48+BaseY,0.0));
  RagdollBodies[4]:=CreateBoxBody(Physics,Vector3(0.195,0.045,0.045),Matrix4x4Translate(0.66,1.48+BaseY,0.0));
  RagdollBodies[5]:=CreateBoxBody(Physics,Vector3(0.20,0.05,0.05),Matrix4x4Translate(-0.36,1.48+BaseY,0.0));
  RagdollBodies[6]:=CreateBoxBody(Physics,Vector3(0.195,0.045,0.045),Matrix4x4Translate(-0.66,1.48+BaseY,0.0));
 end else begin
 ...
 // Elbows and knees.
 if ElbowKneeAsBallSocket then begin
  TKraftConstraintJointBallSocket.Create(Physics,RagdollBodies[3],RagdollBodies[4],Vector3(0.51,1.48+BaseY,0.0),false);
  TKraftConstraintJointBallSocket.Create(Physics,RagdollBodies[5],RagdollBodies[6],Vector3(-0.51,1.48+BaseY,0.0),false);
  TKraftConstraintJointBallSocket.Create(Physics,RagdollBodies[7],RagdollBodies[8],Vector3(0.10,0.48+BaseY,0.0),false);
  TKraftConstraintJointBallSocket.Create(Physics,RagdollBodies[9],RagdollBodies[10],Vector3(-0.10,0.48+BaseY,0.0),false);
 end else begin
  HingeLimitsEnabled:=Limits in [slFull,slNoConeTwist];
  TKraftConstraintJointHinge.Create(Physics,RagdollBodies[3],RagdollBodies[4],Vector3(0.51,1.48+BaseY,0.0),Vector3(0.0,0.0,1.0),HingeLimitsEnabled,false,0.0,2.4,0.0,0.0,false);
  TKraftConstraintJointHinge.Create(Physics,RagdollBodies[5],RagdollBodies[6],Vector3(-0.51,1.48+BaseY,0.0),Vector3(0.0,0.0,1.0),HingeLimitsEnabled,false,-2.4,0.0,0.0,0.0,false);
  TKraftConstraintJointHinge.Create(Physics,RagdollBodies[7],RagdollBodies[8],Vector3(0.10,0.48+BaseY,0.0),Vector3(1.0,0.0,0.0),HingeLimitsEnabled,false,0.0,2.4,0.0,0.0,false);
  TKraftConstraintJointHinge.Create(Physics,RagdollBodies[9],RagdollBodies[10],Vector3(-0.10,0.48+BaseY,0.0),Vector3(1.0,0.0,0.0),HingeLimitsEnabled,false,0.0,2.4,0.0,0.0,false);
 end;
...
var Physics:TKraft;
    ...
    MaxAngVel,MaxLinVel,AngVel,LinVel,PeakAngVel:TKraftScalar;
    ExplodedAt:longint;
begin
 ...
 try
  ...
  for StepIndex:=1 to StepCount do begin
   ...
   if ((MaxAngVel>200.0) or (MaxLinVel>100.0)) and (ExplodedAt<0) then begin
   ...
   if (StepIndex mod 600)=0 then begin
    WriteLn('  t=',(StepIndex/120.0):0:1,'s maxAngVel=',MaxAngVel:0:2,' maxLinVel=',MaxLinVel:0:2,' worst=',BodyNames[WorstIndex],
            ' w=(',RagdollBodies[WorstIndex].AngularVelocity.x:0:1,',',RagdollBodies[WorstIndex].AngularVelocity.y:0:1,',',RagdollBodies[WorstIndex].AngularVelocity.z:0:1,')');
   end;
  end;
  if ExplodedAt<0 then begin
   WriteLn('  STABLE for ',(StepCount/120.0):0:0,'s, peak angvel ',PeakAngVel:0:2);
  end;
 finally
  Physics.Free;
 end;
...
begin
 ...
 WriteLn('=== TGS native / no limits at all (P2P shoulders + bare hinges) ===');
 RunScenario('a',ksmTGSSoft,ktjmNativeSoft,slNoLimits);

 WriteLn('=== TGS native / no limits, elbows+knees as ball sockets (pure P2P everywhere) ===');
 ElbowKneeAsBallSocket:=true;
 RunScenario('b',ksmTGSSoft,ktjmNativeSoft,slNoLimits);
 ElbowKneeAsBallSocket:=false;

 WriteLn('=== TGS native / no limits, arms as unrotated boxes ===');
 UseBoxArms:=true;
 RunScenario('c',ksmTGSSoft,ktjmNativeSoft,slNoLimits);
 UseBoxArms:=false;

 WriteLn('=== TGS native / full limits, arms as unrotated boxes ===');
 UseBoxArms:=true;
 RunScenario('d',ksmTGSSoft,ktjmNativeSoft,slFull);
 UseBoxArms:=false;
end.
