...
procedure RunStack(const aName:string;const aSolverMode:TKraftSolverMode;const aTGSJointMode:TKraftTGSJointMode);
...
begin
 ...
 try
  ...
  WriteLn(aName,': standing=',StandingCount,'/10 topY=',TopY:7:3,' maxTopDrift=',MaxDrift:7:3,' allAsleep=',AllAsleep,' recycled=',RecycledSum,'/',PairSum,' pair-steps');
 finally
  Physics.Free;
 end;
end;
...
begin
 ...
 RunStack('SI     ',ksmSequentialImpulse,ktjmAdapter);
 RunStack('TGS-NAT',ksmTGSSoft,ktjmNativeSoft);
 RunStack('TGS-ADP',ksmTGSSoft,ktjmAdapter);
end.
