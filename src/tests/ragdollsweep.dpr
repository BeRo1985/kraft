...
procedure RunSweep(const aName:string;const aSolverMode:TKraftSolverMode;const aTGSJointMode:TKraftTGSJointMode);
const StepCount=3600;
var Physics:TKraft;
...
begin
 ...
 for Variant:=0 to 19 do begin
  ...
  try
   ...
   WithHinge:=true;
   LimitMargin:=0.10;
   TossJitterX:=Variant*0.007;
   TossJitterZ:=(Variant-10)*0.004;
   BuildScene(Physics);
   PeakAngVel:=0.0;
   LateMax:=0.0;
   Exploded:=false;
   ...
   if Exploded then begin
    inc(CountBad);
    WriteLn('  variant ',Variant:2,': EXPLOSIVE peak=',PeakAngVel:0:1,' late=',LateMax:0:2);
   end else if LateMax>1.0 then begin
    inc(CountRestless);
    WriteLn('  variant ',Variant:2,': restless peak=',PeakAngVel:0:1,' late=',LateMax:0:2);
   end;
  finally
   Physics.Free;
  end;
 end;
 WriteLn(aName,': ',20-CountBad-CountRestless,'/20 clean, ',CountRestless,' restless, ',CountBad,' explosive');
end;

begin
 FormatSettings.DecimalSeparator:='.';
 WriteLn('=== SI sweep ===');
 RunSweep('SI ',ksmSequentialImpulse,ktjmAdapter);
 WriteLn('=== NAT sweep ===');
 RunSweep('NAT',ksmTGSSoft,ktjmNativeSoft);
end.
